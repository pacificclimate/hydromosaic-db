...
def get_outlets(nc, sesh):
    ...
    if "nbasins" not in nc.variables["basin_name"].dimensions:
        raise Exception(f"{exception_prefix}basin_name does not have an nbasins dimension")
...
def get_variables(nc, sesh):
    ...
    for var in nc.variables:
        if timeseries_variable(var):
            if "units" in nc.variables[var].ncattrs() and "long_name" in nc.variables[var].ncattrs():
                units = nc.variables[var].units
                long_name = nc.variables[var].long_name
                
                var_match = sesh.query(Variable).filter(Variable.standard_name == var).first()
                if not var_match:
                    ...
                    raise Exception(f"{exception_prefix}{var} already present in database with different metadata")                    
            else:
...
def get_model(nc, sesh, gcm_prefix):
    ...
    if not model_match:
        ...
        raise Exception(f"{exception_prefix}{model_name} already in the database with a different institution than {model_institution}")
...
def get_scenario(nc, sesh, gcm_prefix):
    ...
    scenario_match = sesh.query(Scenario).filter(Scenario.short_name == scenario_short).first()
    
    if not scenario_match:
        logger.info(f"{scenario_short} scenario added to database")
        s = Scenario(
            short_name = scenario_short,
            long_name = scenario_long
            )
        sesh.add(s)
        return s
        ...
        raise Exception(f"{exception_prefix}{scenario_short} already in the database with a different long name than {scenario_long}")
...
def index_directory(dsn, directory, log_level, gcm_prefix):
    ...
    session.commit()
    session.close()
    logger.info(f"Successfully indexed {len(indexed)} files and {num_timeseries} timeseries.")
    for file in indexed:
